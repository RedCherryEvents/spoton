 ...
 *
 * Auth: `authorizeCron` — `x-cron-secret` or `Authorization: Bearer`
 * matching `AUTOMATION_CRON_SECRET` / `CRON_SECRET`. The two
 * endpoints (`/api/automations/cron` and this one) are independent
 * operations; we keep them on separate URLs so one failing doesn't
 * block the other.
 *
