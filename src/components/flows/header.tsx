...
export function EditorHeader() {
  ...
  function goBack() {
    if (dirty && !window.confirm(t("leaveUnsavedConfirm"))) return;
    router.push("/flows");
  }
  ...
  return (
    <div className="flex flex-col gap-1.5 px-6 pt-5">
      ...
      <Dialog open={confirmDelete} onOpenChange={setConfirmDelete}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{tList("deleteTitle")}</DialogTitle>
            <DialogDescription>
              {tList("deleteDesc", { name: state.name })}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
